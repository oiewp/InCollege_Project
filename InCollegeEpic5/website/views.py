...
from .models import User, Friend, Job, Experience, Education
...
def create_profile():
    ...
    if request.method == 'POST':
        ...
        currUser = User.query.filter_by(accountName=current_user.accountName).first()

        currUser.title = title
        currUser.major = major
        currUser.university = university
        currUser.about = about

        # set hasInfo as false if any variables have not been set to True
        if currUser.title and currUser.major and currUser.university and currUser.about:
            currUser.hasInfo = True
        else:
            currUser.hasInfo = False
        ...
        if education:
            ...
            currUser.hasEdu = True
            db.session.add(newEdu)

        if schoolName and degree and years:
            currUser.hasEdu = True
        else:
            currUser.hasEdu = False

        if currUser.hasEdu and currUser.hasInfo and currUser.hasExp:
            currUser.hasProfile = True
        else:
            currUser.hasProfile = False
...
@views.route('/view_friend_profile', methods=['POST'])
@login_required
def view_friend_profile():
    print("in view friend profile")
    friendName = request.json.get('friend_user_name')
    print(friendName)
    friend = User.query.filter_by(accountName=friendName).first()
    print(friend.accountName)
